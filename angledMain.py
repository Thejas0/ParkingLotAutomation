...
from angledHelperLaneConstruction import *
from angledHelperPreprocessing import *
from angledHelperPrinting import *
from angledHelperVissim import *

blockWidth = BLOCKWIDTH
blockLength = BLOCKLENGTH
parkWidth = blockWidth*math.sqrt(2)
parkLength = PARKINGLENGTH
parkingLength = 1.414*2*blockLength - 0.2

length = LENGTH
width = WIDTH
roadWidth = blockWidth
pLength = 2
pWidth = 1
isDecimal = False
    ...
    for i in range(len(lines)):
        ...
        if (line[0].strip() == "Entry cell"):
            ...
            if (row == 0):
                ...
                startLink = encodeGrid(
                    row+length, col, 0, grid, ct, connected, length)
                ct += 1
                endLink = encodeGrid(row+width, col+length-1,
                                     180, grid, ct, connected, length)
                ct += 1
...
with open(fileName, 'r') as file:
    ...
    for i in range(len(lines)):
        ...
        if (method == 'Drive'):
            encodeGrid(row, col, angle, grid, ct, connected, length)
            ct += 1
            # print()
print("----------------------------")
printListGrid(grid)
...
connectUTurnInit(gridReduced, width, isDecimal, roadWidth, count,start)
...
if (start[0] == 0):
    ...
    baseLink = addBaseLink(
        baseindex[0], baseindex[1], 0, grid, ct, 200, roadWidth)
...
# ----------------
preProcessParking(gridReducedCopy, filename, length, pLength, pWidth)
print("COMPLETED PREPROCESSING")

# -------------------------
parkingLotinit(gridReducedCopy, baseLink, roadWidth,
               pWidth, pLength, parkWidth, parkingLength)
# -------------------------
