...
"""
    change appropriate filename and no of rows and coll accordingly
"""
# ----------------------------------------------------------------
# ----------------------------------------------------------------
# ----------------------------------------------------------------
...
# NO OF ROW AND COL ACCORDING TO INPUT CHANGE ACCORDINGLY
rows, cols = (50+2*length, 50+2*length)
...
def connectUTurn(gridR, check, angle1, reqAngle, item1, x, y, z):
    global count
    ...
    for k in range(l):
        print(x1, y1, k)
        print(gridR[x1][y1], len(grid[x1][y1]))
        ...
        if (angle2 == reqAngle):
            ...
            fillUturnDriveLane(gridR, x, y, 2*width, fillTemp, count, reqAngle)
            ...
            for k in range(len(gridR[i][j])):
                ...
                if (m == 'SD'):
                    ...
                    if (angle1 == 270):
                        ...
                        # check = [0, 1]
                        check = [0, width]
                        connectUTurn(gridR, check, angle1,
                                     reqAngle, item, i, j, k)
                    elif (angle1 == 90):
                        reqAngle = 270
                        # check = [0, -1]
...
print("length ", len(gridReduced[7][10]))
